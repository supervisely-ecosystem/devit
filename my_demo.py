...
def main(
    ...
    model_path="weights/vitl_0069999.pth",
    image_dir="project/dataset_1/img",
    output_dir="output/dataset_1",
    category_space="hummingbird_prototypes.pth",
    device="cuda",
    ...
    config.DE.CLASS_PROTOTYPES = (
        "./weights/lvis_v1_known_train.vitl14.pkl,weights/lvis_v1_novel_train.vitl14.pkl"
    )
    config.DE.BG_PROTOTYPES = "./weights/background_prototypes.vitl14.pth"
    ...
    for img_file in glob(osp.join(image_dir, "*")):
        ...
        # scaler = max(image.shape[0], image.shape[1]) / 1080
        # resized_height = int(image.shape[0] / scaler)
        # resized_width = int(image.shape[1] / scaler)
        # image = torch.from_numpy(image)
        # image = image.unsqueeze(0)
        # image = image.unsqueeze(0)
        # image = torch.nn.functional.interpolate(
        #     image, (resized_height, resized_width, 3), mode="nearest"
        # )
